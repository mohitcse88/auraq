...
export const HomePage: React.FC = () => {
  const { currentTheme, products, setActivePage, setSelectedCategory, setIsDesignModalOpen } = useStore();
  ...
  return (
    <div className="space-y-16 sm:space-y-24">
      {/* Dynamic Design Option Banner for User Prompt Fulfillment */}
      <section className="bg-neutral-50 border-b border-neutral-200 py-3 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 text-neutral-700">
            ...
            />
            <span className="font-semibold text-neutral-900">Active Design:</span>
            <span className="font-medium">{currentTheme.name}</span>
            <span className="text-neutral-400 hidden md:inline">({currentTheme.subtitle})</span>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-neutral-500 hidden sm:inline">
              Need a different aesthetic vibe?
            </span>
            <button
              onClick={() => setIsDesignModalOpen(true)}
              className="px-3 py-1 font-semibold uppercase tracking-wider text-[11px] text-white transition-opacity shadow-sm"
              style={{
